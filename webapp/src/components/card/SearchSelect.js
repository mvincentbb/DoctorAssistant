...
class SearchSelect extends React.Component {
    ...
    _select_option = (elt) => {
        ...
        // var p_act = window.$(elt).parent().parent().parent().find(".selecionado_opcion").find('input').val(window.$(elt).text());
        this.setState({searchText: window.$(elt).text()});
    ...
    handleSeachTextClick = (e) => {
        e.stopPropagation();
        window.$(e.target).parent().trigger("click");
    }
    ...
    render() {
        ...
        var filteredOptions = this.props.options.filter( (option) => {
            return option.value.toLowerCase().indexOf(this.state.searchText.toLowerCase()) !== -1;
        })

        return (
            <div className="select_mate" data-mate-select="active" data-indx-select={this.props.value} data-selec-open="false" style={{minHeight: '35px'}}>
				<select>
                ...
                <p className="selecionado_opcion" onClick={ this.open_select } data-n-select={this.props.value} style={{padding: '4px 2%'}} >
                    <input value={this.state.searchText} onClick={ this.handleSeachTextClick } onChange={ this.handleSeachTextChange } style={{display: 'inline-block', width: '96%'}} />
                </p>
